...
> **En corto:** la fecha prometida se calculaba con una sola función que mezclaba dos problemas
> distintos: **qué extras contrató el cliente** (varían en cantidad — hoy son banderas booleanas) y
> **qué contrato tiene** (varía en tipo — estándar, garantía, corporativo). Dos razones de cambio en
> el mismo lugar: la violación de SRP que el H2 ya había señalado.
...
> **En corto:** fusionamos **Decorator + Strategy**. El Decorator arma el servicio contratado apilando
> extras y produce `horas()`; la Strategy consume esas horas y las convierte en fecha prometida según
> el contrato. Conviven por un solo dato: `politica.calcular(recepcion, prioridad, servicio.horas())`.
...
> **En corto:** descartamos **Observer + Decorator** ([`h3/con-observer/`](../h3/con-observer/)),
> **Observer + Strategy** y **meter los extras dentro de la Strategy**. Las tres por la misma razón de
> fondo: eran patrones puestos uno al lado del otro, no conectados por un dato.
...
> **En corto:** **ganamos** que un extra nuevo no toca ninguna política y un contrato nuevo no toca
> ningún extra — el acoplamiento entre los dos patrones es un solo número. **Pagamos** 9 clases donde
> había una función con `if`, y el cálculo repartido en dos lugares: para responder "¿por qué esta
> orden vence el jueves?" hay que mirar el servicio armado *y* la política aplicada.
